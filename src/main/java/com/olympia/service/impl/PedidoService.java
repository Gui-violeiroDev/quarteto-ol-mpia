...
public class PedidoService {
    ...
    public List<PedidoResponse> listarPorUsuario(String email) {
        // Busca tanto pelo usuário logado quanto pelo email do cliente
        Usuario usuario = usuarioRepository.findByEmail(email).orElse(null);
        if (usuario != null) {
            List<Pedido> porUsuario = pedidoRepository.findByUsuarioId(usuario.getId());
            if (!porUsuario.isEmpty()) return porUsuario.stream().map(this::toResponse).toList();
        }
        // Fallback: busca pelo email do cliente
